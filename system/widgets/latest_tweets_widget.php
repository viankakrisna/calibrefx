    ...
    function widget($args, $instance) {
        ...
        if (!$tweets) {
            ...
            $twitter = wp_remote_retrieve_body(
                    wp_remote_request(
                            sprintf('http://api.twitter.com/1/statuses/user_timeline.json?screen_name=%s&count=%s&trim_user=1', $instance['twitter_id'], $count), array('timeout' => 100,)
                    )
            );
            ...
            if (!$twitter) {
                ...
                foreach ((array) $json as $tweet) {
                    ...
                    $tweets[] = '<li>' . calibrefx_tweet_linkify($tweet->text) . ' <span class="twitter-time">' . $timeago_link . '</span></li>' . "\n";
                }
